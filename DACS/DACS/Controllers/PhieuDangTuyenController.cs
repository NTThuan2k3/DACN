...
namespace DACS.Controllers
{
	public class PhieuDangTuyenController : Controller
	{
		...
		public async Task<IActionResult> Index(string tinh, string nganh, string namKN, string luong, string tenVL, string chucDanh)
		{
			var list = await _phieuDangTuyenRepository.GetAllAsync();
			ViewBag.FilterTinhs = _context.Tinhs.OrderBy(p => p.TenTinh).Where(p => p.StatusId == 1).ToList();
			ViewBag.FilterNganhs = _context.PhieuDangTuyens.OrderBy(p => p.TenNganh).ToList();
			ViewBag.FilterNamKNs = _context.PhieuDangTuyens.OrderBy(p => p.NamKN).ToList();
			ViewBag.FilterLuongs = _context.PhieuDangTuyens.OrderBy(p => p.MucLuong).ToList();
			ViewBag.FilterTenViecLams = _context.PhieuDangTuyens.OrderBy(p => p.TenViecLam).ToList();
			ViewBag.FilterChucDanhs = _context.PhieuDangTuyens.OrderBy(p => p.ChucDanh).ToList();
			...
			ViewBag.SelectedNamKN = namKN;
			ViewBag.SelectedLuong = luong;
			ViewBag.SelectedTenVL = tenVL;
			ViewBag.SelectedChucDanh = chucDanh;
			...
			if (namKN != null)
			{
				list = list.Where(s => s.NamKN == namKN && s.StatusId == 1).ToList();
			}
			if (luong != null)
			{
				list = list.Where(s => s.MucLuong == luong && s.StatusId == 1).ToList();
			}
			if (tenVL != null)
			{
				list = list.Where(s => s.TenViecLam == tenVL && s.StatusId == 1).ToList();
			}
			if (chucDanh != null)
			{
				list = list.Where(s => s.ChucDanh == chucDanh && s.StatusId == 1).ToList();
			}
			return View(list);
		}
		...
		public async Task<IActionResult> Details(int id, string tinh, string nganh)
		{
			...
			ViewBag.CV = new SelectList(cvs, "Id", "TenCV");
