...
namespace DACS.Controllers
{
	public class PhieuDangTuyenController : Controller
	{
		...
		public async Task<IActionResult> Index(string tinh, string nganh, string namKN, string luong, string tenVL, string chucDanh)
		{
            ...
            ViewBag.FilterNganhs = _context.PhieuDangTuyens.OrderBy(p => p.TenNganh).Distinct().ToList();
            ViewBag.FilterNamKNs = _context.PhieuDangTuyens.OrderBy(p => p.NamKN).Distinct().ToList();
            ViewBag.FilterLuongs = _context.PhieuDangTuyens.OrderBy(p => p.MucLuong).Distinct().ToList();
            ViewBag.FilterTenViecLams = _context.PhieuDangTuyens.OrderBy(p => p.TenViecLam).Distinct().ToList();
            ViewBag.FilterChucDanhs = _context.PhieuDangTuyens.OrderBy(p => p.ChucDanh).Distinct().ToList();
